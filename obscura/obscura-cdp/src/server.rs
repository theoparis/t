...
pub async fn start(port: u16) -> anyhow::Result<()> {
    start_with_options(port, None, false, None, None).await
}
    ...
    local
        .run_until(async {
            ...
            let processor_handle =
                tokio::task::spawn_local(cdp_processor(msg_rx, proxy, stealth, ja3, ja4));
...
async fn cdp_processor(
    ...
    let mut ctx = CdpContext::new_with_options(proxy, stealth, ja3, ja4);
    ...
    loop {
        ...
        tokio::select! {
            ...
            Some(intercepted) = async {
                if let Some(mut irx) = intercept_rx {
                    irx.recv().await
                } else {
